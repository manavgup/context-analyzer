...
def test_reconstruct_session_basic():
    messages = [
        ...
        _make_assistant_msg(
            3, "Found the bug, fixing now", ts="2026-06-01T10:00:10Z", input_tokens=32000, output_tokens=300
        ),
    ]
...
def test_detect_compactions_from_api_multiple():
    ...
    turns = [
        ConversationTurn(
            ...
            timestamp="T1",
            user_prompt_text="t1",
            api_calls=[ApiCall(api_call_index=0, conversation_turn=1, input_tokens=3, cache_creation_tokens=30000)],
        ),
        ...
        ConversationTurn(
            ...
            timestamp="T2",
            user_prompt_text="t2",
            api_calls=[ApiCall(api_call_index=1, conversation_turn=2, input_tokens=3, cache_creation_tokens=50000)],
        ),
        ConversationTurn(
            turn_number=3,
            timestamp="T3",
        ...
        ConversationTurn(
            ...
            timestamp="T4",
            user_prompt_text="t4",
            api_calls=[ApiCall(api_call_index=3, conversation_turn=4, input_tokens=3, cache_creation_tokens=80000)],
        ),
    ]
    result = _detect_compactions_from_api(turns)
    assert result == [2, 4]
...
def test_reconstruct_session_compaction_creates_epochs():
    ...
    summary_blocks = [bid for bid in snap_3.block_ids if block_registry[bid].block_type == BlockType.COMPACTION_SUMMARY]
...
def test_reconstruct_session_blocks_after_compaction():
    ...
    pre_non_pinned = [
        bid
        for bid in snap_pre.block_ids
        if not block_registry[bid].is_pinned and block_registry[bid].block_type != BlockType.COMPACTION_SUMMARY
    ]
    post_non_pinned_non_summary = [
        bid
        for bid in snap_post.block_ids
        if not block_registry[bid].is_pinned and block_registry[bid].block_type != BlockType.COMPACTION_SUMMARY
    ]
